...
```{r}
#my_path = "simulated_data/data.csv"
my_path = "nwu_data/PatientObservations.csv"
...
First, select only the ICD concepts
```{r}
...
map_charlson_codes <- function(data) { 
  ...
  mapped_codes_table <- rbind(icd10_mapped_table, icd9_mapped_table)
...
```{r}
comorb_list_names <- map_charlson_codes(data)[[1]]
icd_map <- map_charlson_codes(data)[[2]]
index_scores <- map_charlson_codes(data)[[3]]
table1 <- map_charlson_codes(data)[[4]]
mapped_codes_table <- map_charlson_codes(data)[[5]]
```
