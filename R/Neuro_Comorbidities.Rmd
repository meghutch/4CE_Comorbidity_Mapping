...
```{r message=FALSE, warning=FALSE}
neuro <- read.csv("neuro/neuro_icd.csv")
colnames(neuro)[3] <- "concept_code"

# Import Sites Phase2.1 LocalPatientSummary and LocalPatientObservations files
pt_summary <- read.csv("../nu4ce/Phase2.1/LocalPatientSummary.csv")
obs <- read.csv("../nu4ce/Phase2.1/LocalPatientObservations.csv")
...
```{r}
obs <- obs %>% filter(concept_type == "DIAG-ICD10")

neuro_pts <- obs %>% 
  filter(days_since_admission >= 0) 
neuro_pts <- merge(neuro_pts, neuro, by = 'concept_code')
...
combined_table1$Full_Cohort_perc <- round(combined_table1$Full_Cohort/nrow(pt_summary)*100,2)

combined_table1$Neuro_Cohort_perc <- round(combined_table1$Neuro_Cohort/nrow(pt_summary)*100,2)
